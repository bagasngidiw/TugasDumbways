...
func main() {
	...
	//  post
	e.POST("/", middleware.UploadFile(addFormProject)) //ini yg di mau add project
	e.POST("/editProject/:id", middleware.UploadFile(editFormProject))
	e.POST("/deleteProject/:id", deleteProject)
	// e.POST("/deleteProject/:id", middleware.UploadFile(deleteProject))
...
func home(c echo.Context) error {
	...
	data, _ := connection.Conn.Query(context.Background(), "SELECT tb_project.id, author_id, project_name, start_date, end_date, description, image, technologies, tb_user.name AS author FROM public.tb_project LEFT JOIN tb_user ON tb_project.author_id = tb_user.id ORDER BY tb_project.id DESC")
	...
	for data.Next() {
		...
		err := data.Scan(&each.Id, &each.AuthorId, &each.ProjectName, &each.StartDate, &each.EndDate, &each.Description, &each.Image, &each.Technologies, &each.Author)
		if err != nil {
	...
	if sess.Values["isLogin"] != true {
		...
		dataUser.IsLogin = true //sess.Values["isLogin"].(bool)
		dataUser.Name = sess.Values["name"].(string)
		dataUser.Id = sess.Values["Id"].(int)
	}
...
func login(c echo.Context) error {
	...
	if err != nil {
		return redirectWithMessage(c, "Email Invalid!", false, "/form-login")
	}

	// check apakah hashedpassword dan pw yg dimasukin sudah sama di db
	errPassword := bcrypt.CompareHashAndPassword([]byte(user.Password), []byte(password))

	if errPassword != nil {
		return redirectWithMessage(c, "Password Invalid!", false, "/form-login")
	}

	sess, _ := session.Get("session", c)
	sess.Options.MaxAge = 10800 // 3 jam waktu login
...
func formRegister(c echo.Context) error {
	...
	if sess.Values["isLogin"] != true {
		...
		dataUser.IsLogin = sess.Values["isLogin"].(bool)
		dataUser.Name = sess.Values["name"].(string)
	}
...
func myProject(c echo.Context) error {
	...
	data, _ := connection.Conn.Query(context.Background(), "SELECT id, project_name, start_date, end_date, description, image, technologies FROM tb_project WHERE author_id=$1", author_id)
	// data, _ := connection.Conn.Query(context.Background(), "SELECT tb_blog.id, tb_user.name, tb_blog.title, tb_blog.content, tb_blog.image, tb_blog.post_date FROM tb_blog LEFT JOIN tb_user ON tb_blog.author_id = tb_user.id")
...
func projectDetail(c echo.Context) error {
	...
	err := connection.Conn.QueryRow(context.Background(), "SELECT tb_project.id, project_name, start_date, end_date, description, image, technologies, tb_user.name AS author FROM public.tb_project JOIN tb_user ON tb_project.author_id = tb_user.id WHERE tb_project.id=$1", id).Scan(&ProjectDetail.Id,
		&ProjectDetail.ProjectName, &ProjectDetail.StartDate, &ProjectDetail.EndDate, &ProjectDetail.Description, &ProjectDetail.Image,  
		&ProjectDetail.Technologies, &ProjectDetail.Author,
	)
...
func projectEdit(c echo.Context) error {
	...
	errQuery := connection.Conn.QueryRow(context.Background(), "SELECT id, project_name, start_date, end_date, description, technologies FROM tb_project WHERE id=$1", id).Scan(&ProjectDetail.Id,
...
func addFormProject(c echo.Context) error {
	...
	_, err := connection.Conn.Exec(context.Background(),
		"INSERT INTO tb_project ( project_name, start_date, end_date, description, technologies, image, author_id) VALUES($1, $2, $3, $4, $5, $6, $7)",
		projectName, startDate, endDate, description, technologies, image, author,
	)
...
func editFormProject(c echo.Context) error {
	...
	_, err := connection.Conn.Exec(context.Background(),
		"UPDATE tb_project SET project_name=$1, start_date=$2, end_date=$3, description=$4, technologies=$5, image=$6 WHERE id=$7",
		projectName, start, end, description, technologies, image, id,
	)
